...
export default function ChatWidget({ settings }: { settings: BotSettings | null }) {
  ...
  useEffect(() => {
    const welcomeText = settings?.welcome_message || "Hello! How can I help you today?";
          ...
          <div className="flex-grow p-4 overflow-y-auto space-y-4">
            {messages.map((message, index) => (
              <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div 
                  style={{ backgroundColor: message.role === 'user' ? primaryColor : undefined }} 
                  className={`px-4 py-2 rounded-lg max-w-xs ${message.role === 'user' ? 'text-white' : 'bg-gray-200 text-gray-800'}`}
                >
